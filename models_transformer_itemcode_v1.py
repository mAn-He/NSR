...
from engines.week16_engines_itemcode_v1 import *
import matplotlib
matplotlib.use('Agg')
...
class MultiModalModel(nn.Module):
    def __init__(self, num_images_per_data=2,
        ...
        if image_embeddings_dim_out is not None:
            self.image_model_head = nn.Linear(in_features=self.image_model.fc.in_features, out_features=image_embeddings_dim_out)
            self.image_output_dim = image_embeddings_dim_out
        else:
            self.image_output_dim = self.image_model.fc.in_features
            self.image_model_head = nn.Identity()
        ...
        # 막단 layer
        self.input_dim = self.image_output_dim + self.text_embedding_dim + self.other_features_dim
    ...
    def forward(self, images, text, other_features):
        image_embeddings = self.activ(self.image_model_head(images))
