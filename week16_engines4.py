...
def train(model,optimizer,criterion, eval_metric,train_dataloader, test_dataloader,loss_ratio:float=1, scheduler=None,device='cpu',num_epochs=50,dir_path='model_state_dict',a=None,b=None,boxcox_lambda=None):
    ...
    train_loss_unscaled_list=[]
    test_loss_unscaled_list = []
    ...
    for epoch in range(num_epochs):
        ...
        with torch.inference_mode():
            for i, batch in tqdm(enumerate(test_dataloader),desc='Testing',leave=False):
                images,text_embeddings, other_embeddings, targets_unscaled, targets_scaled = [x.to(device) for x in batch]
                outputs_scaled = model(images, text_embeddings, other_embeddings)
                eval_scaled = eval_metric(outputs_scaled.flatten(), targets_scaled.flatten())
                loss_scaled = criterion(outputs_scaled.flatten(), targets_scaled.flatten())
        ...
        test_loss_unscaled_list.append(test_loss_unscaled/len(test_dataloader))
        ...
        if torch.isnan(outputs_unscaled).any() != True:
            if test_loss_unscaled/len(test_dataloader) < best_loss:
                best_loss = test_loss_unscaled/len(test_dataloader)
                print(best_loss)
                torch.save(model.state_dict(), dir_path+'/'+'model.pt')
                print(f"Saved best model at epoch {epoch + 1} with unnormalized Target Loss: {best_loss:.4f}")
       
        if scheduler:
            scheduler.step()
        
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(train_eval_scaled_list)), train_eval_scaled_list, label='Train Eval Normalized')
        plt.xlabel('Epochs')
        plt.ylabel('Eval Score')
        plt.legend()
        plt.savefig(dir_path+'/'+'train_eval_normalized_plot.png')
        plt.close()
        
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(test_eval_scaled_list)), test_eval_scaled_list, label='Test Eval Normalized')
        plt.xlabel('Epochs')
        plt.ylabel('Eval Score')
        plt.legend()
        plt.savefig(dir_path+'/'+'test_eval_normalized_plot.png')
        plt.close()
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(train_loss_scaled_list)), train_loss_scaled_list, label='Train loss Normalized')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend()
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(test_loss_scaled_list)), test_loss_scaled_list, label='Test loss Normalized')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend()
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(train_loss_unscaled_list)), train_loss_unscaled_list, label='Train loss UnNormalized')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig(dir_path+'/'+'train_unnormalized_loss.png')
        plt.close()
        
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(test_loss_unscaled_list)), test_loss_unscaled_list, label='Test loss UnNormalized')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig(dir_path+'/'+'test_unnormalized_loss.png')
        plt.close()
