...
class nsr_img_txt_dataset(Dataset):
    def __init__(self, dataframe, transform=None):
        ...
        self.image_model = torchvision.models.resnet152(weights=torchvision.models.ResNet152_Weights.DEFAULT)
    ...
    def __getitem__(self, idx):
        ...
        for image_tensor in images:
            ...
            with torch.no_grad():
                resnet_output = self.image_model(image_tensor)
            resnet_outputs.append(resnet_output)

        resnet_outputs = torch.stack(resnet_outputs)
        summed_resnet_outputs = resnet_outputs.sum(dim=0)
        ...
        if target_scaled is not None:
            ...
            return summed_resnet_outputs, text_embeddings_tensor, other_embeddings_tensor, target_tensor, target_scaled_tensor

        else:
            return summed_resnet_outputs, text_embeddings_tensor, other_embeddings_tensor, target_tensor
...
def train(model,optimizer,criterion, eval_metric,train_dataloader, test_dataloader,loss_ratio:float=1, scheduler=None,device='cpu',num_epochs=50,dir_path='model_state_dict',a=None,b=None,boxcox_lambda=None):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in tqdm(enumerate(train_dataloader),desc='Training',leave=False):
            ...
            # print('targets_unscaled shape:',targets_unscaled.shape)
            ...
            # print('outputs_unscaled shape:',outputs_unscaled.shape)
        ...
        if torch.isnan(outputs_unscaled).any() != True:
            if test_loss_scaled/len(test_dataloader) < best_eval:
                best_eval = test_loss_scaled/len(test_dataloader)
                torch.save(model.state_dict(), dir_path+'/'+'model.pt')
                print(f"Saved best model at epoch {epoch + 1} with Unscaled Target Loss: {best_eval:.4f}")
