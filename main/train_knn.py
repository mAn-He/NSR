...
df_train['판매수량_scaled'] = np.round(y_train_boxcoxed_with_shifting,4)
df_test['판매수량_scaled'] = np.round(y_test_boxcoxed_with_shifting,4)
...
df_train = df_train.drop(columns=['Unnamed: 0', '판매시작연도', '판매첫날', '상품코드', '판매일자', '상품명', '상품명2', '칼라', '칼라명', '칼라명2',
                                  '현재가', '할인율(%)', '파일경로', '이미지갯수', '외관설명', '기능설명','카테고리',f'{args.k_neighbors} closest idx'],errors='ignore')
...
other_features_dim_in = df_train.drop(df_train.filter(like='설명').columns,axis=1).shape[-1]-4
...
model = MultiModalModel(num_images_per_data=2,
                        ...
                        knn_embedding_dim=args.k_neighbors,
