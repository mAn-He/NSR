...
def train(model,optimizer,criterion, eval_metric,train_dataloader, test_dataloader,loss_ratio:float=1, scheduler=None,device='cpu',num_epochs=50,dir_path='model_state_dict',a=None,b=None,boxcox_lambda=None):
    ...
    for epoch in range(num_epochs):
        ...
        plt.plot(range(len(train_eval_scaled_list)), train_eval_scaled_list, label='Train Eval Scaled')
        plt.plot(range(len(test_eval_scaled_list)), test_eval_scaled_list, label='Test Eval Scaled')
        plt.xlabel('Epochs')
        plt.ylabel('Eval Score')
        plt.legend()
        plt.savefig(dir_path+'/'+'eval_scaled_plot.png')
        plt.close()
        
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(train_eval_unscaled_list)), train_eval_unscaled_list, label='Train Eval UnScaled')
        plt.plot(range(len(test_eval_unscaled_list)), test_eval_unscaled_list, label='Test Eval Unscaled')
        plt.xlabel('Epochs')
        plt.ylabel('Eval Score')
        plt.legend()
        plt.savefig(dir_path+'/'+'eval_unscaled_plot.png')
        plt.close()
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig(dir_path+'/'+'scaled_loss.png')
        plt.close()
